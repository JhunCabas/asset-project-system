<?php
	...
	class User{
		...
		function getEditPassword($id,$pass)
		{
			$Opass = md5($pass);
			$newPass = md5($this->getPassword());
			return mysql_query('UPDATE user SET password="'.$newPass.'" WHERE id="'.$id.'" AND password="'.$Opass.'"');
		}
		
		function getUserId($uname)
		{
			
			return mysql_query('SELECT id,username,password,firstName,lastName,position,status FROM user WHERE username = "'.$uname.'" LIMIT 1');
		}
