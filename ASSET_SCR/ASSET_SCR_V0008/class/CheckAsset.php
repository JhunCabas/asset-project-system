<?php
	...
	class CheckAsset{
		...
		function getCheckAssetByYear($assetAddDate)
		{
			...
			return mysql_query('SELECT B.id,B.assetYear,B.assetAddDate,B.assetTypeCode,B.assetGroupCode,B.assetCode,A.assetName,A.assetPrice,A.assetLocation,B.status FROM asset AS A,checkasset AS B WHERE A.assetYear = "'.$assetAddDate.'" AND B.assetTypeCode = A.assetTypeCode AND B.assetGroupCode = A.assetGroupCode AND B.assetCode = A.assetCode ORDER BY B.assetAddDate DESC,B.assetTypeCode,B.assetGroupCode,B.assetCode ASC');
		}
