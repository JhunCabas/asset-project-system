<?php
	...
	class CheckAsset{
		...
		function getCheckAsset()
		{
			...
			return mysql_query('SELECT B.id,B.assetYear,B.assetAddDate,B.assetTypeCode,B.assetGroupCode,B.assetCode,A.assetName,A.assetPrice,A.assetLocation,B.status FROM asset AS A,checkasset AS B WHERE B.assetAddDate = A.assetAddDate AND B.assetTypeCode = A.assetTypeCode AND B.assetGroupCode = A.assetGroupCode AND B.assetCode = A.assetCode ORDER BY B.assetAddDate DESC,B.assetTypeCode,B.assetGroupCode,B.assetCode ASC');
		}
		...
		function getCheckAssetByYear($assetAddDate)
		{
			...
			return mysql_query('SELECT B.id,B.assetYear,B.assetAddDate,B.assetTypeCode,B.assetGroupCode,B.assetCode,A.assetName,A.assetPrice,A.assetLocation,B.status FROM asset AS A,checkasset AS B WHERE A.assetAddDate LIKE "%'.$assetAddDate.'%" AND B.assetTypeCode = A.assetTypeCode AND B.assetGroupCode = A.assetGroupCode AND B.assetCode = A.assetCode ORDER BY B.assetAddDate DESC,B.assetTypeCode,B.assetGroupCode,B.assetCode ASC');
		}
		...
		function checkAssetStatus()
		{
			...
			return mysql_query('UPDATE checkasset SET status="'.$this->getStatus().'" WHERE id="'.$this->id.'"');
		}
