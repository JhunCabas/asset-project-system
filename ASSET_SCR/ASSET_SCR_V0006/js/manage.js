...
function getCheck(id)
	{
		...
		$.getJSON("../request/reqSelCheck.php",{'id':id},function(data)
		{
			...
			$.each(data,function(key,val)
			{
				...
				$('#dialog').html("<table><tr><td>ประจำปี  : "+val['assetYear']+"</td></tr>"+
				"<tr><td>วัน/เดือน/ปี ที่ได้มา  &nbsp&nbsp&nbsp: "+val['assetAddDate']+"</td></tr>"+
				"<tr><td>เลขครุภัณฑ์  &nbsp&nbsp&nbsp: "+val['yearShort']+"/"+val['assetTypeCode']+"-"
				+val['assetTypeCode']+""+val['assetGroupCode']+
				"-"+val['assetCode']+"</td></tr>"+
				"<tr><td>ชื่อครุภัณฑ์  &nbsp&nbsp&nbsp: "+val['assetName']+"</td></tr>"+
				"<tr><td>วันที่เช็คล่าสุด  &nbsp&nbsp&nbsp: "+val['checkDate']+"</td></tr>"+
				"<tr><td>สถานะ  &nbsp&nbsp&nbsp: "+val['status']+"</td></tr>"+
				"</table></br></br><center>"+
				"<td style='text-align:center; width: 130px;'><a class='button big icon approve' onClick=''>ใช้งานได้</a>"+
				"<td style='text-align:center; width: 130px;'><a class='button big icon approve' onClick=''>ชำรุด</a>"+
				"<td style='text-align:center; width: 130px;'><a class='button big icon approve' onClick=''>เสื่อมสภาพ</a></center>");
...
function GenAssetCode(){
    ...
    var selectedGroup = selectBox.options[selectBox.selectedIndex].value;
	var year = $('#addAssetAddDate').val().substring(8,10);
	var yearS = $('#addAssetAddDate').val().substring(6,10);
	var group = selectedType+selectedGroup;

	
	if($('#addAssetCode').val()==""){
		$.get("../request/reqGenAsset.php",{'addYear':yearS,
