<?php
	...
	class CheckAsset{
		...
		function getCheckAsset()
		{
			...
			return mysql_query('SELECT D.checkId,C.assetId,D.assetYear,D.assetAddDate,A.assetTypeCode,B.assetGroupCode,C.assetCode,C.assetName,C.assetPrice,C.assetLocation,D.checkStatus FROM assettype AS A, assetgroup AS B, asset AS C, checkasset AS D WHERE D.assetTypeId = A.assetTypeId AND D.assetGroupId = B.assetGroupId AND D.assetId = C.assetId ORDER BY D.assetYear DESC,A.assetTypeCode,B.assetGroupCode,C.assetCode ASC');
		}
		
		function getCheckAssetLimit($st,$end)
		{
			
			return mysql_query('SELECT D.checkId,C.assetId,D.assetYear,D.assetAddDate,A.assetTypeCode,B.assetGroupCode,C.assetCode,C.assetName,C.assetPrice,C.assetLocation,D.checkStatus FROM assettype AS A, assetgroup AS B, asset AS C, checkasset AS D WHERE D.assetTypeId = A.assetTypeId AND D.assetGroupId = B.assetGroupId AND D.assetId = C.assetId ORDER BY D.assetYear DESC,A.assetTypeCode,B.assetGroupCode,C.assetCode ASC LIMIT '.$st.','.$end.'');
		}
		
		function getCheckAssetByStatus($status)
		{
			
			return mysql_query('SELECT D.checkId,D.assetYear,D.assetAddDate,A.assetTypeCode,B.assetGroupCode,C.assetCode,C.assetName,C.assetPrice,C.assetLocation,D.checkStatus FROM assettype AS A,assetgroup AS B,asset AS C,checkasset AS D WHERE D.assetTypeId = A.assetTypeId AND D.assetGroupId = B.assetGroupId AND D.assetId = C.assetId AND D.checkStatus = "'.$status.'" ORDER BY D.assetYear DESC,A.assetTypeCode,B.assetGroupCode,C.assetCode ASC');
		}
		
		function getCheckAssetByStatusLim($status,$st,$end)
		{
			
			return mysql_query('SELECT D.checkId,D.assetYear,D.assetAddDate,A.assetTypeCode,B.assetGroupCode,C.assetCode,C.assetName,C.assetPrice,C.assetLocation,D.checkStatus FROM assettype AS A,assetgroup AS B,asset AS C,checkasset AS D WHERE D.assetTypeId = A.assetTypeId AND D.assetGroupId = B.assetGroupId AND D.assetId = C.assetId AND D.checkStatus = "'.$status.'" ORDER BY D.assetYear DESC,A.assetTypeCode,B.assetGroupCode,C.assetCode ASC LIMIT '.$st.','.$end.'');
		}
