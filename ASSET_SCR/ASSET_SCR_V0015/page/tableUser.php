...
//****************************************************************//
//****************************************************************//
//** ชื่อไฟล์ : tableUser.php 
//** คำอธิบาย : ไฟล์นี้  แสดงตาราง user
//** Version : 1.0
	...
	while($res = $db->fetch_array($arr))
{
	if($res['userStatus']=="Administrator"){
		...
		<a class='button icon edit' onClick='getEditUser(".$res['userId'].");'></a></td>";
		if($_SESSION["id"]==$res['userId']){
			echo "<td>&nbsp;</td>";
		}
		else{
			echo "<td id='Udel".$no."' style='text-align:center'>
			<a class='button icon remove danger' onClick='delUser(".$res['userId'].");'></a></td>";
		}
		echo "</tr>";
		$no++;
	}
