<?php
	...
	class Asset{
		...
		function getAsset()
		{
			...
			return mysql_query('SELECT C.assetId,C.assetCode,C.assetName,C.assetPrice,A.assetTypeCode,B.assetGroupCode,C.assetYear,C.assetAddDate,C.remark FROM assettype AS A,assetgroup AS B,asset AS C WHERE C.assetTypeId = A.assetTypeId AND C.assetGroupId=B.assetGroupId ORDER BY C.assetYear DESC,C.assetTypeId,C.assetGroupId,C.assetCode ASC');
		}
