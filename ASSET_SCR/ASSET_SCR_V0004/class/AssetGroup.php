<?php
	...
	class AssetGroup{
		...
		function checkAssetGroup()
		{
			...
			return mysql_query('SELECT id,assetGroupCode,assetGroupName,assetTypeCode FROM assetgroup WHERE assetTypeCode="'.$this->getAssetTypeCode().'"');
		}
		...
		function editAssetGroup()
		{
			...
			return mysql_query('UPDATE assetgroup SET assetGroupName="'.$this->getAssetTypeName().'",assetTypeCode="'.$this->getAssetTypeAddDate().'" WHERE id="'.$this->id.'"');
		}
